...
if 'library_management.book' not in odoo.env:
    raise HTTPException(status_code=500, detail="The model 'library_management.book' does not exist in the database")
...
def add_book(book: Book):
    try:
        book_id = odoo.env['library_management.book'].create({
            'name': book.name,
            'author': book.author,
...
def show_book(book_id: int, lan: str | None):
    ...
    try:
        book = odoo.env['library_management.book'].browse(book_id)
        if not book.exists():
...
def show_books():
    try:
        books = odoo.env['library_management.book'].search_read([], ['name', 'author', 'published_date', 'isbn'])
        return books
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def update_book(book_id: int, book: Book):
    try:
        book_record = odoo.env['library_management.book'].browse(book_id)
